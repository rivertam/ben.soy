//! Content-hashed serving for the Thought calculator wasm pair.
//!
//! `/thoughts-core.js` is the stable, `no-cache` loader imported by the
//! calculator and combobox adapters. The generated glue and wasm routes are immutable
...
fn bytes_response(content_type: &'static str, cache: &'static str, body: Vec<u8>) -> Response {
    Response::builder()
        ...
        .body(Body::from(body))
        .expect("static Thought calculator wasm headers")
}
