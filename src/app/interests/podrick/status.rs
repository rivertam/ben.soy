...
use benjisponge::data::{Data, Db, podrick_models::PodrickAnnouncement};

/// What the page shows about job 1. A database that is unreachable or has
/// never been written yields an empty summary rather than an error page —
/// `/podrick` is mostly prose and must render without the bot running.
...
pub async fn load(data: &Data) -> AnnounceSummary {
    let Ok(db) = data.db().await else {
        return AnnounceSummary::default();
    };
