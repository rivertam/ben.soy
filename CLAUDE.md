...
- Fitness writes are create-only except `DELETE /api/fitness/workouts/by-path/{path}` (sync token OR admin cookie + same-origin), the admin weight editor `POST /fitness/exercise/{name}`, and interruption CRUD (`POST /fitness/interruptions`, `POST /fitness/interruptions/{id}`, `POST /fitness/interruptions/{id}/delete` — admin cookie + same-origin; annotate-only, no scoring impact). Deletes remove the workout and its sets only — `exercises`/`exercise_tags`/`exercise_muscles` orphans are left deliberately (invisible to every count, and they preserve corrected taxonomy/weights across a delete-and-repaste)
