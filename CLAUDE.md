...
Rust SSR personal site on topcoat 0.6.2 — a niche framework; read
...
- `just build` — required Fitness Wasm + cargo build + `topcoat asset bundle`; serving without the bundle step panics
- `just fitness-wasm` — build the required Fitness entry worker; `just wasm` builds it plus Diary's optional worker. wasm-bindgen CLI must equal pinned 0.2.126; read `docs/diary-sync.md` before touching `crates/` or sync routes. `crates/diary-worker` is deliberately its OWN workspace carrying a wasm-only SurrealDB patch — never merge it into the root workspace
- `just check` — fmt + clippy -D warnings + tests; must pass before claiming done
- `just deploy` — optional Railway redeploy + Cloudflare cache purge (Railway GitHub App deploys on push to main; CI runs `just fitness-wasm` plus `just check`). Prod path: `docs/railway-deploy.md` (Dockerfile only — not Railpack); DNS/Tunnel/CDN: `docs/cloudflare-deploy.md`
...
- `/` is the log page plus, below 40rem, the phone pane deck — log ⇄ felix ⇄ fitness ⇄ resume ⇄ more as scroll-snap panes (`/log` 308s home with its query; felix/fitness/resume render through shared content components, so page-body changes reach both URLs). Deck panes (`home.rs`) and tab bar (`chrome.rs::PANE_TABS`) are test-aligned; phone chrome lives in `styles/panes.css`
...
- Fitness data is database data, not content: never hardcode the CSV or runs; changes spanning `/fitness`, import, API, schema, tags, or local startup must preserve `docs/fitness.md` invariants
- Records (`/fitness` lifting badges) are derived from set history at snapshot build (`src/app/interests/lifting/archive/records.rs`), never stored or imported — there is deliberately no records table
- Fitness writes are create-only except date-keyed step upserts at `POST /api/fitness/steps` (`STEPS_SYNC_TOKEN`), `DELETE /api/fitness/workouts/by-path/{path}` (sync token OR admin cookie + same-origin), the admin weight/identity editors at `POST /fitness/exercise/{name}` and `POST /fitness/exercise/{name}/identity`, and interruption CRUD (`POST /fitness/interruptions`, `POST /fitness/interruptions/{id}`, `POST /fitness/interruptions/{id}/delete` — admin cookie + same-origin; annotate-only, no scoring impact). Deletes remove the workout and its sets only — `exercises`/`exercise_tags`/`exercise_muscles` orphans are left deliberately (invisible to every count, and they preserve corrected taxonomy/weights across a delete-and-repaste)
...
- Topcoat 0.6 skips path-scoped layers on unmatched requests, so the whole-site response layer is deliberately pathless and manually registered in `app.rs`; viewer no-store and em-dash rewriting all stay in `src/app/response_layer.rs`, never a sibling layer
...
- `/fitness/entry` is local-first but not offline-readable: `fitness-entry-core` owns draft/validation/guidance/queue policy, `/fitness/sw.js` owns IndexedDB and publication but has no `fetch` handler, and pending predicted links are never hosted links. Read `docs/fitness.md` + the Fitness PWA section of `docs/auth.md` before touching it
